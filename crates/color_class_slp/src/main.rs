//! This program runs the Multiclass Single-Layer Perceptron
//! algorithm on a set of data using specified training data.
//! The resulting classifications are printed to the terminal.
//!
...
/// Runs the Multiclass Single-Layer Perceptron algorithm and outputs
/// the results.
///
...
fn main() {
    ...
    if args.len() < 3 || args.len() > 5 {
        ...
        writeln!(lock, "Multiclass Single-Layer Perceptron").unwrap();
        writeln!(lock, "Author: Benjamin Hall").unwrap();
        writeln!(
            lock,
            "Usage: ./color_class_slp [train data file name] [test data file name] [learning rate = 1.0] [threshold = 0.0]"
        )
        .unwrap();
        writeln!(lock).unwrap();
        ...
        writeln!(
            lock,
            "Runs the multiclass single-layer perceptron algorithm on a set of data using the given training data and learning rate."
        )
        .unwrap();
        writeln!(
            lock,
    ...
    // run multiclass single-layer perceptron algorithm
    let test_res = data_utils::classify::multiclass_single_layer_perceptron(
        &train_data,
        &test_data,
        learning_rate,
        threshold,
